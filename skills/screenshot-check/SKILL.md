...
- **アニメーションの瞬間は要素スクショ(`locator.screenshot()`)では撮れない** — Playwrightは要素が静止するまで待ってから撮影するため、常に完了後の姿が写る。動きの検証は次の合わせ技で行う:
  1. `page.screenshot()`(viewport全体)を150〜200ms間隔で連写する(こちらは静止を待たない)
  2. `page.evaluate` でアニメ用クラスの付与やパーティクル要素の数をassertする(見た目でなくDOMで確認)
  3. 費用や時間のかかる処理(画像生成・AI呼び出し等)の完了演出は、状態ファイル/APIに擬似データを注入して状態遷移だけ再現する。**終わったら必ず原状復帰する**
